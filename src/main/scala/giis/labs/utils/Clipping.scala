...
class Clipping(graphicsScene: GraphicsScene) {
    ...
    def clip(polygonList: List[Point], pointList: List[Point]) {
        var point1 = pointList.head
        var point2 = pointList.tail.head

        var listLine = List[giis.labs.model.Point]()
        listLine = point1 :: listLine
        listLine = point2 :: listLine

        var shapeLine = ShapeFactory.createShape(listLine, Shape.LineBrezenhem)
        ...
        var dirV = new PointFloat(point2.x - point1.x, point2.y - point1.y)
        var visible = true
        var i = 0

        while (i < polygonList.size && visible) {

            var q0 = new PointFloat(point1.x - polygonArray.apply(i).x, point1.y - polygonArray.apply(i).y)
            var qi = dotProduct(normals.apply(i), q0)
            var pi = dotProduct(normals.apply(i), dirV)
        ...
        if (t1 <= t2 && t1 >= 0 && t2 <= 1) {
            var p11 = new Point(math.round(point1.x.toFloat + t1 * dirV.x.toFloat), math.round(point1.y.toFloat + t1 * dirV.y.toFloat))
            var p22 = new Point(math.round(point1.x.toFloat + t2 * dirV.x.toFloat), math.round(point1.y.toFloat + t2 * dirV.y.toFloat))
    ...
    def getNormals(polygonList: Array[Point]): Array[PointFloat] = {
        ...
        for (i <- 0 to (polygonList.length - 1)) {
            ...
            val p1 = new PointFloat(-(polygonList.apply(j).y.toFloat - polygonList.apply(i).y.toFloat) / (polygonList.apply(j).x.toFloat - polygonList.apply(i).x.toFloat), 1)
            val v1 = new PointFloat(polygonList.apply(k).x.toFloat - polygonList.apply(i).x.toFloat, polygonList.apply(k).y.toFloat - polygonList.apply(i).y.toFloat)
